...
def run_game(q):
  ...
  if flag:
      r = requests.post(f"http://127.0.0.1:8000/users_flap/",data ={'name':name,'score':score})
  else:
      r = requests.put(f"http://127.0.0.1:8000/users/flap/{name}",data ={'name':name,'score':max(score,pastScore)})
